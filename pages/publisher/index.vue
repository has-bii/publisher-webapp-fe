            ...
            <div class="card shadow border-light mb-4">
                ...
                <div class="card-body">
                    <div class="mb-3">
                        ...
                        <p v-else class="fs-4 fw-bold">{{ publisher ? publisher.name : 'No publisher is selected' }}</p>
                    </div>
                    <div class="mb-3">
                        <div class="fs-6 text-secondary">Publisher Email:</div>
                        <p v-if="$fetchState.pending" class="fs-4">Loading</p>
                        <p v-else-if="$fetchState.error" class="fs-4">Error</p>
                        <p v-else class="fs-5 fw-bold">{{ publisher ? publisher.email : 'No publisher is selected' }}</p>
                    </div>
                </div>
            </div>
                ...
                <div class="card-body">
                    ...
                    <div class="row">
                        <div class="col">
                            ...
                            <select v-else class="form-select" @change="fetch" v-model="selected_publisher">
                                <option selected disabled value="null">Select publisher..</option>
                                <option v-for="mypublisher in my_publishers" :value="mypublisher.id">{{ mypublisher.name }}
                                </option>
                            </select>
                        </div>

                        <div class="col-2">
                            <button type="button" class="btn btn-primary" @click="fetch">Select</button>
                        </div>
                    </div>
            ...
            <div class="card shadow border-light mb-4">
                <div class="card-header py-3 d-flex flex-row align-items-center justify-content-between">
                    ...
                    <h6 v-else class="m-0 font-weight-bold text-primary">
                        Member of {{ publisher ? publisher.name : '' }}
                    </h6>
                </div>
                ...
                <div class="card-body">
                    <div class="table-responsive">
                        <table class="table">
                            ...
                            <tbody v-else-if="!members" class="table-group-divider">
                                <tr>
                                    <td colspan="12" class="text-center"> No publisher is selected. </td>
                                </tr>
                            </tbody>
                            <tbody v-else class="table-group-divider">
...
export default {
    ...
    async fetch() {
        ...
        await this.fetch_myPublisher()
    },
    methods: {
        async fetch() {
            await this.$axios.get('publisher', {
                params: {
                    id: this.selected_publisher
                }
            })
                .then(({ data }) => {
        ...
        async fetch_myPublisher() {
            await this.$axios.get('get-users', {
                ...
                .then(({ data }) => {
                    this.my_publishers = data.result.publishers
                })
        },
        async addMember(event) {
            ...
            if (!this.selected_publisher) {
                ...
                this.toast.text = 'No publisher is selected!'
            ...
            await this.$axios.post('/publisher/add-member', {
                ...
                publisher_id: this.selected_publisher
            })
                .then(({ }) => {
